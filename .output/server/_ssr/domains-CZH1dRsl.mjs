...
const getDomain = (slug) => DOMAINS.find((d) => d.slug === slug);
const filterProjectSlugs = (slug) => {
  const d = getDomain(slug);
  if (!d) return null;
  return new Set(d.projects);
};
const filterAppSlugs = (slug) => {
  const d = getDomain(slug);
  if (!d) return null;
