import { T as TSS_SERVER_FUNCTION, c as createServerFn } from "./server-6YljCchu.mjs";
...
const MODEL_CHAIN = ["google/gemini-2.5-flash", "google/gemini-2.5-flash-lite", "openai/gpt-5-mini"];
...
function smartFallback(messages) {
  ...
  const langHint = /\[lang=(fa|en|ar|tr)\]/i.exec(last)?.[1];
  const lang = langHint ?? detectLang(last);
  const intents = [{
    key: "price",
    fa: /قیمت|هزینه|تعرفه|چقدر/,
    en: /price|cost|how much|pricing/,
    ar: /سعر|تكلفة|كم/,
    tr: /fiyat|ücret|maliyet|ne kadar/
  }, {
    key: "contact",
    fa: /تماس|ارتباط|واتساپ|تلگرام|شماره|ایمیل/,
    en: /contact|reach|phone|email|telegram|whatsapp/,
    ar: /تواصل|اتصال|هاتف|بريد|تيليجرام/,
    tr: /iletişim|telefon|e-?posta|telegram/
  }, {
    key: "thesis",
    fa: /پایان.?نامه|تز|دکتر|ارشد|مقاله|isi|q1/i,
    en: /thesis|phd|msc|master|paper|publication|isi|q1/,
    ar: /أطروحة|ماجستير|دكتوراه|مقال|بحث/,
    tr: /tez|yüksek lisans|doktora|makale|yayın/
  }, {
    key: "industrial",
    fa: /صنعت|شرکت|کارخانه|سازمان|industrial/,
    en: /industrial|enterprise|factory|company/,
    ar: /صناع|شركة|مصنع/,
    tr: /endüstri|kurumsal|fabrika|şirket/
  }, {
    key: "app",
    fa: /اپلیکیشن|اپ|موبایل|اندروید|ios/i,
    en: /app|mobile|android|ios|flutter/,
    ar: /تطبيق|موبايل|أندرويد/,
    tr: /uygulama|mobil|android|ios/
  }, {
    key: "chatbot",
    fa: /چت.?بات|chatbot|llm|rag|بات/i,
    en: /chatbot|bot|llm|rag|assistant/,
    ar: /روبوت|محادثة|llm|rag/i,
    tr: /chatbot|bot|llm|rag/
  }, {
    key: "greet",
    fa: /^(سلام|درود|های)/,
    en: /^(hi|hello|hey|salam)\b/,
    ar: /^(مرحبا|أهلا|سلام)/,
    tr: /^(merhaba|selam|hi|hello)/
  }];
  const matched = intents.find((i) => i[lang]?.test?.(q));
...
const chatConsult_createServerFn_handler = createServerRpc({
  id: "5d03932537243d107b0e7c53e8305b7fe661ae80a427942cdc5dda664865ba53",
  name: "chatConsult",
  filename: "src/lib/chat.functions.ts"
}, (opts) => chatConsult.__executeServer(opts));
...
}).handler(chatConsult_createServerFn_handler, async ({
  data
}) => {
  const apiKey = process.env.LOVABLE_API_KEY;
  if (!apiKey) {
    return {
      reply: smartFallback(data.messages),
      fallback: true
    };
  }
  let lastStatus = 0;
  for (const model of MODEL_CHAIN) {
    try {
      ...
      if (res.ok) {
        ...
        const reply2 = json?.choices?.[0]?.message?.content ?? "";
        if (reply2.trim()) return {
          reply: reply2,
          fallback: false,
          model
        };
...
export {
  chatConsult_createServerFn_handler
};
